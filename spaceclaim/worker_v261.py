...
def create_export_options(settings):
    ...
    version_name = settings.get("acis_version", "V5")
    if version_name != "V5":
        raise RuntimeError("SpaceClaim 2026 R1 exports ACIS V5; requested " + str(version_name))
...
def validate_output(path):
    ...
    if path.lower().endswith(".sat"):
        if not header.startswith(b"500 "):
            raise RuntimeError("unexpected SAT version: expected ACIS 5.0")
    elif path.lower().endswith(".sab"):
        if (not header.startswith(b"ACIS BinaryFile") or len(header) < 19
                or struct.unpack("<I", header[15:19])[0] != 500):
            raise RuntimeError("unexpected SAB version: expected ACIS 5.0")
    else:
        raise RuntimeError("unsupported ACIS output extension")
    return size
